...
class ChatGPTTelegramBot:
    ...
    def __init__(self, config: dict, openai: OpenAIHelper):
        ...
        self.commands = [
            ...
            BotCommand(command='reset', description='Перезагрузка бота '
                                                    '(/reset Команда для перезагрузки)'),
            BotCommand(command='image', description='Генерация картинки по запросу (Команда /image кот)'),
            BotCommand(command='stats', description='Статистика'),
        ]
        self.disallowed_message = "Нужно обновить ресурсы " 
                                  
        self.budget_limit_message = "Лимит исчерпан"
        self.usage = {}

    async def delete_messages(bot, chat_id, timeout=43200):
        while True:
            messages = bot.messages(chat_id=chat_id)
            ...
            time.sleep(300) # Проверка каждые 30 минут
        
        delete_messages(bot, chat_id)
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        help_text = 'Я твой новый асистент' + \
                    '\n\n' + \
                    '\n'.join(commands) + \
                    '\n\n' + \
                    'Так же могу выдать транскрипцию голосового сообщения' + \
                    '\n\n' + \
                    "Удачи"
